...
9. **Transition to implementation** — invoke `writing-plans-for-teams` to create the implementation plan

**The terminal state is invoking `writing-plans-for-teams`.** This plugin (superpowers-lite-teams) routes the full-lane brainstorming terminal through the team fitness check instead of going directly to `writing-plans`. `writing-plans-for-teams` evaluates whether parallel team execution pays off; if it does, it writes a team-format plan; if it doesn't, it falls back to `writing-plans` for the standard serial flow. Either way, plans that should be serial still are.

Do NOT invoke `writing-plans` directly, `frontend-design`, `mcp-builder`, or any other implementation skill. The ONLY skill you invoke after brainstorming is `writing-plans-for-teams`.
...
- Invoke the `writing-plans-for-teams` skill to create the implementation plan. It runs a team fitness check and either produces a team-format plan or falls back to `writing-plans` for serial work.
- Do NOT invoke any other skill. `writing-plans-for-teams` is the next step.
