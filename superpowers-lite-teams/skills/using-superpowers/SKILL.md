...
Full lane is bit-for-bit the upstream superpowers methodology, with one addition specific to this plugin: **Agent Teams parallel execution is enabled by default.** After brainstorming, planning routes through `writing-plans-for-teams`, which runs a four-criterion fitness check (≥4 tasks, ≥2 waves with ≥2 tasks each, ≥2 distinct specialist roles, no pervasive shared state). If the plan passes, it gets a team-format plan and executes via `agent-team-driven-development` with persistent specialist implementers in parallel waves. If it fails the fitness check, it gracefully falls back to `writing-plans` + `subagent-driven-development` — the standard serial flow — so plans that should be serial still are.

**Prerequisites for team execution to actually run:**
- Claude Code ≥ 2.1.32
- `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in `~/.claude/settings.json`

If the experimental flag is missing, `agent-team-driven-development` halts at boot with a remediation pointer. The fitness-fallback path (serial subagent-driven execution) works without the flag.
