...
…then collapse to a **single reviewer pass**: after the implementer subagent commits, dispatch only the spec-compliance reviewer. Skip the code-quality reviewer for that task. The final whole-implementation reviewer at the end of the plan still runs and catches quality issues that span tasks.
